...
class DB
{
	...
	public function AutoIncrement($tableName){
		...
		$this->query('ALTER TABLE '.$tableName.' AUTO_INCREMENT = 1');
		$AutoIncrement =  $this->row("SELECT `AUTO_INCREMENT` as Auto_increment FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '".$this->DBName."' AND TABLE_NAME = '".$tableName."'");
